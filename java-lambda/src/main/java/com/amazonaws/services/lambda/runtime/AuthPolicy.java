...
public class AuthPolicy {
  ...
  public AuthPolicy(final String principalId, final PolicyDocument policyDocumentObject) {
  ...
  /**
   * IAM Policies use capitalized field names, but Lambda by default will serialize object members
   * using camel case
   *
   * This method implements a custom serializer to return the IAM Policy as a well-formed JSON
   * document, with the correct field names
   *
  ...
  public Map<String, Object> getPolicyDocument() {
    ...
    final Statement[] statements = policyDocumentObject.getStatement();
    final Map<String, Object>[] serializableStatementArray = new Map[statements.length];
    for (int i = 0; i < statements.length; i++) {
      final Map<String, Object> serializableStatement = new HashMap<>();
      final AuthPolicy.Statement statement = statements[i];
  ...
  public void setPolicyDocument(final PolicyDocument policyDocumentObject) {
    this.policyDocumentObject = policyDocumentObject;
  }
   ...
   *
   * Initialize the PolicyDocument with the region where the RestApi is configured, the AWS Account
   * ID that owns the RestApi, the RestApi identifier and the Stage on the RestApi that the Policy
   * will apply to
   */
  public static class PolicyDocument {
     ...
     */
    public PolicyDocument(final String region, final String awsAccountId, final String restApiId,
        final String stage) {
      this.region = region;
    ...
    public void allowMethod(final HttpMethod httpMethod, final String resourcePath) {
      addResourceToStatement(allowStatement, httpMethod, resourcePath);
    }

    public void denyMethod(final HttpMethod httpMethod, final String resourcePath) {
      addResourceToStatement(denyStatement, httpMethod, resourcePath);
    }

    public void addStatement(final AuthPolicy.Statement statement) {
      statements.add(statement);
    }

    private void addResourceToStatement(final Statement statement, final HttpMethod httpMethod,
        String resourcePath) {
      ...
      final String method = httpMethod == HttpMethod.ALL ? "*" : httpMethod.toString();
      statement.addResource(String.format(EXECUTE_API_ARN_FORMAT, region, awsAccountId, restApiId,
     ...
     */
    public static PolicyDocument getAllowOnePolicy(final String region, final String awsAccountId,
        final String restApiId, final String stage, final HttpMethod method,
        final String resourcePath) {
      final AuthPolicy.PolicyDocument policyDocument =
          new AuthPolicy.PolicyDocument(region, awsAccountId, restApiId, stage);
     ...
     */
    public static PolicyDocument getDenyOnePolicy(final String region, final String awsAccountId,
        final String restApiId, final String stage, final HttpMethod method,
        final String resourcePath) {
      final AuthPolicy.PolicyDocument policyDocument =
          new AuthPolicy.PolicyDocument(region, awsAccountId, restApiId, stage);
    ...
    public static AuthPolicy.PolicyDocument getAllowAllPolicy(final String region,
        final String awsAccountId, final String restApiId, final String stage) {
      return getAllowOnePolicy(region, awsAccountId, restApiId, stage, HttpMethod.ALL, "*");
    }

    public static PolicyDocument getDenyAllPolicy(final String region, final String awsAccountId,
        final String restApiId, final String stage) {
  ...
  static class Statement {
    ...
    public Statement(final String effect, final String action, final List<String> resourceList,
    ...
    public static Statement getEmptyInvokeStatement(final String effect) {
    ...
    public void addCondition(final String operator, final String key, final Object value) {
