...
aws lambda update-function-configuration \
  ...
  --handler $HANDLER \
  --environment "Variables={STAGE=$STAGE,AK_APP_ID=$AK_APP_ID,AK_APP_VERSION=$AK_APP_VERSION,AK_APP_SECRET=$AK_APP_SECRET,DATABASE_URL=$DATABASE_URL,DATABASE_USER=$DATABASE_USER,DATABASE_PASS=$DATABASE_PASS,TWILIO_ACCOUNT_SID=$TWILIO_ACCOUNT_SID,TWILIO_AUTH_TOKEN=$TWILIO_AUTH_TOKEN}" \
  --memory-size 1536 \
  --profile $PROFILE
