          ...
          <div className="row">
            <div className="media col-md-12 col-sm-6">
              ...
              <div className="media-body gma-orange-border" style={{padding: '20px'}}>
                ...
                <h4>Care for kids ages: <span className="normal">{gma.careAges.map((age) => CareAge.parse(age).text).join(", ")}</span></h4>
                <h4>Provide care at: <span className="normal">{gma.careLocations.map((item) => CareLocation.parse(item).text).join(', ')}</span></h4>
                <h4>General Availability: <span className="normal">{gma.availabilities.map((item) => Availability.parse(item).text).join(', ')}</span></h4>
              </div>
            </div>
          </div>
          <div className="row" style={{marginTop: '10px'}}>
            <div className="media col-md-12 col-sm-6">
              <div className="media-body gma-orange-border" style={{padding: '0px 20px'}}>
                <h4>I would describe myself as: <span className="normal">{gma.demeanors.map((item) => Demeanor.parse(item).text).join(', ')}</span></h4>
              ...
              <div className="media-body gma-orange-border" style={{padding: '0px 20px'}}>
                ...
                <ul>
                  {gma.careExperiences.map((exp) => {
                    return <li key={exp}><h4><span className="normal">{CareExperience.parse(exp).text}</span></h4></li>
                  })}
                </ul>
              </div>
            </div>
              ...
              <div className="media-body gma-orange-border" style={{padding: '0px 20px'}}>
                ...
                <ul>
                  {gma.careTrainings.map((tra) => {
                    return <li key={tra}><h4><span className="normal">{CareTraining.parse(tra).text}</span></h4></li>
                  })}
                </ul>
              </div>
            </div>
